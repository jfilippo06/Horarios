...
class CargaAcademica(tk.Toplevel):
	...
	def gestionarMaterias(self):
		if self.tree.selection():
			...
			ttk.Label(self.new, text='MATERIAS',font=('Helvetica',14)).place(x=620,y=5)
			...
			frameNewButton.grid(row=1,column=0)
			ttk.Button(frameNewButton, text='ASIGNAR MATERIA', command=self.registrarMateria).grid(row=0,column=0)
